        ...
        doctorsHTML += `
            <div class="swiper-slide">
                <div class="card h-100 border-0 shadow-sm doctor-card">
                    <img src="${doctor.photo || '/static/images/doctors/default.jpg'}" 
                         class="card-img-top doctor-img" 
                         alt="${doctor.surname} ${doctor.name} ${doctor.patronymic}">
                    <div class="card-body">
                        <h5 class="card-title text-green doctor-name">${doctor.surname} ${doctor.name} ${doctor.patronymic}</h5>
                        ...
                        <p class="card-text small text-primary mt-2">
                            <i class="bi bi-geo-alt me-1"></i>${doctor.clinic}
                        </p>
                    </div>
...
function initSwiper() {
    ...
    doctorsSwiper = new Swiper('.doctors-swiper', {
        slidesPerView: 1,
        spaceBetween: 24,
        loop: false, // Убираем loop для правильного отображения
        centeredSlides: false, // Отключаем центрирование
        autoplay: false, // Отключаем автоплей для начала
        pagination: {
